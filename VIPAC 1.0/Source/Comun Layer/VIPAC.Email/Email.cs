...
namespace VIPAC.Email
{
    public class Email
    {
        ...
        public Email To(string emailAddress)
        {
            if (!string.IsNullOrWhiteSpace(emailAddress))
            {
                foreach (string str in emailAddress.Split(new[] {';'}).Where(p => !string.IsNullOrWhiteSpace(p)))
                {
        ...
        {
            var strArray = name.Split(new[] {';'}).Where(p => !string.IsNullOrWhiteSpace(p)).ToArray();
            var strArray2 = emailAddress.Split(new[] {';'}).Where(p => !string.IsNullOrWhiteSpace(p)).ToArray();
